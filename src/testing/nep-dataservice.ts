...
import { komtDocentOvereen, losDocentIdentiteitOp } from '../app/utils/docent-identiteit';
...
export class NepDataService {
  ...
  async bulkSaveLeerlingen(
    items: { id?: string; data: Omit<Leerling, 'id'> }[],
  ) {
    for (const item of items) {
      if (item.id) {
        await this.updateLeerling(item.id, item.data);
      } else {
        await this.addLeerling(item.data);
      }
    }
  }
  ...
  async saveDocent(docent: Docent) {
    ...
    this.docenten.update(l => {
      ...
      return bestaand
        ? l.map(d =>
            d.afkorting === docent.afkorting ? compleet : d,
          )
        : [...l, compleet];
    });
  }
  ...
  async deleteDocent(afkorting: string) {
    ...
    this.docenten.update(l =>
      l.filter(d => d.afkorting !== afkorting),
    );
  }
  ...
  async updateDocentVak(
    id: string,
    updates: Partial<DocentVak>,
  ) {
  ...
  async bulkDeleteDocentVakken(ids: string[]) {
    for (const id of ids) {
      await this.deleteDocentVak(id);
    }
  }

  async bulkSaveDocentVakken(
    items: {
      id?: string;
      data: Omit<DocentVak, 'id'>;
    }[],
  ) {
    for (const item of items) {
      if (item.id) {
        await this.updateDocentVak(item.id, item.data);
      } else {
        await this.addDocentVak(item.data);
      }
    }
  }
  ...
  async updateMemoTW1TW2(
    id: string,
    updates: Partial<MemoTW1TW2>,
  ) {
  ...
  async bulkDeleteMemoTW1TW2(ids: string[]) {
    for (const id of ids) {
      await this.deleteMemoTW1TW2(id);
    }
  }
  ...
  async saveMentorVoorbereiding(
    item: Partial<MentorVoorbereiding> & {
    ...
    if (bestaand?.id) {
      this.controleer(
        'saveMentorVoorbereiding:update',
        bestaand.id,
      );
      ...
      this.controleer(
        'saveMentorVoorbereiding:create',
        id,
      );
  ...
  async toggleLock(
    ...
    const id = `${klas}_${periode}_${schooljaar}`.replace(
      /[^a-zA-Z0-9_-]/g,
      '_',
    );
  ...
  async zetTakenUit(
    ...
    const bestaand = new Set(
      this.docentTaken().map(sleutel),
    );

    const teSchrijven = nieuweTaken.filter(
      taak => !bestaand.has(sleutel(taak)),
    );
